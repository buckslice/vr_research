...
public static class Noise {
    ...
    //private static float[] recVertDists = { Mathf.Abs(1), 1, 1 };

    public static int hashVector(Vector3 input) {
        return (int)(input.x * 73856093) ^ (int)(input.y * 19349663) ^ (int)(input.z * 83492791);
    }

    // fractional brownian motion
    public static float fBM(NoiseMethod method, Vector3 v, float seed, float frequency, int octaves, float persistence = 0.5f, float lacunarity = 2.0f) {
        float total = 0.0f;
        float amplitude = 1f;
        float maxAmp = 0f;

        for (int i = 0; i < octaves; ++i) {
            //float noise = GetNoise(v.x * frequency + seed, v.y * frequency + seed, v.z * frequency + seed);
            NoiseSample noise = method(v + Vector3.one * seed, frequency);
    ...
    // ridged multifractal
    public static float rMF(NoiseMethod method, Vector3 v, float seed, float frequency, int octaves, float persistence = 0.6f, float lacunarity = 2.0f, bool smoothValleys = false) {
        float total = 0.0f;
        float amplitude = 1f;
        for (int i = 0; i < octaves; ++i) {
            NoiseSample noise = method(v + Vector3.one * seed, frequency);
            total += amplitude * (1f - Mathf.Abs(noise.value));
            amplitude *= persistence * (smoothValleys ? Mathf.Clamp01(total) : 1f);
            frequency *= lacunarity;
        }

        return (total - 1f) * 1.25f;
    }
    ...
    // if inbetween then output is blending of the two
